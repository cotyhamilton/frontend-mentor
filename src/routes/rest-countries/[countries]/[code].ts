export async function get({ params }) {
	...
	try {
		const res = await fetch(`https://restcountries.com/v3.1/alpha/${params.code}?fields=name,cca3,flags,flag,currencies,region,subregion,population,tld,languages,capital,borders`);
		const data = await res.json();
		country = data;
	} catch(error) {
		const res = await fetch("https://restcountries.com/v3.1/all?fields=name,cca3,flags,flag,currencies,region,subregion,population,tld,languages,capital,borders");
		const data = await res.json();
		countries = data;
